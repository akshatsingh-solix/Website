...
Configure with environment variables. Any provider whose key is set is used:
  ...
  OPENROUTER_API_KEY  OpenRouter ":free" models           SOL_OPENROUTER_MODELS (falls back to OPENROUTER_MODELS)
  ...
  SOL_PROVIDER_ORDER  comma-separated order, default gemini,groq,mistral,openrouter,custom
"""
import os
import json
import time
...
def configured_providers() -> List[Provider]:
    ...
    if os.environ.get("OPENROUTER_API_KEY"):
        known["openrouter"] = Provider("openrouter", "https://openrouter.ai/api/v1", os.environ["OPENROUTER_API_KEY"],
                                       _models("SOL_OPENROUTER_MODELS", "nvidia/nemotron-3.5-lightning:free", "OPENROUTER_MODELS"),
    ...
    order = [p.strip() for p in os.environ.get("SOL_PROVIDER_ORDER", "gemini,groq,mistral,openrouter,custom").split(",")]
