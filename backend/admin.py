...
CSV_FIELDS = ["id", "created_at", "type", "status", "name", "email", "company", "job_title", "phone", "interest", "role", "resource", "message", "source", "source_page", "notes", "updated_at"]
STATUSES = ("new", "contacted", "qualified")


def _query(type: Optional[str], q: Optional[str], status: Optional[str] = None) -> dict:
    clauses = []
    ...
    if q:
        ...
        clauses.append({"$or": [{"name": rx}, {"email": rx}, {"company": rx}, {"message": rx}, {"interest": rx}, {"role": rx}, {"resource": rx}, {"notes": rx}]})
...
async def stats():
    ...
    return {"total": total, "last_7_days": last_7, "by_type": by_type, "by_status": by_status, "chat_leads": chat_leads, "alerts_sent": alerts_sent}


@router.get("/submissions")
async def list_submissions(type: Optional[str] = None, q: Optional[str] = None, status: Optional[str] = None, page: int = Query(1, ge=1), page_size: int = Query(25, ge=1, le=200)):
    query = _query(type, q, status)
...
async def export_submissions(type: Optional[str] = None, q: Optional[str] = None, status: Optional[str] = None):
    docs = await db.submissions.find(_query(type, q, status), {"_id": 0}).sort("created_at", -1).to_list(10000)
    buf = io.StringIO()
