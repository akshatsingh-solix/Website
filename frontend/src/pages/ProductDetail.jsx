      ...
      <section className="border-b border-white/5">
        <div className="container grid grid-cols-1 divide-y divide-white/10 sm:grid-cols-3 sm:divide-x sm:divide-y-0">
          {product.outcomes.map((o, i) => (
            <Reveal key={o.label} delay={i * 0.08} className="py-10 sm:px-8 first:sm:pl-0">
              <p className={`font-display text-5xl font-medium tracking-tighter ${product.accent === "teal" ? "text-teal" : "text-primary"}`}>{o.value}</p>
              <p className="mt-2 font-mono text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{o.label}</p>
            </Reveal>
          ))}
        </div>
      </section>
