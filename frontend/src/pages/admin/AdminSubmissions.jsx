...
import { useLiveRefresh } from "@/components/admin/kit";

const TYPES = [
  { key: "all", label: "All" }, { key: "demo", label: "Demo" }, { key: "contact", label: "Contact" }, { key: "partner", label: "Partner" },
  { key: "career", label: "Career" }, { key: "download", label: "Download" }, { key: "newsletter", label: "Newsletter" }, { key: "event", label: "Event" }, { key: "chat", label: "Chat" },
];
const STATUSES = [
...
const TYPE_TONE = { chat: "bg-teal/10 text-teal border-teal/30", demo: "bg-primary/15 text-primary-ink border-primary/30", contact: "bg-teal/10 text-teal border-teal/30", partner: "bg-violet-500/10 text-violet-300 border-violet-500/30", career: "bg-emerald-500/10 text-emerald-300 border-emerald-500/30", download: "bg-sky-500/10 text-sky-300 border-sky-500/30", newsletter: "bg-line/5 text-muted-foreground border-line/15", event: "bg-amber-500/10 text-amber-700 border-amber-500/30" };
const PAGE_SIZE = 25;
...
export default function AdminSubmissions() {
  ...
  const load = useCallback(async ({ silent = false } = {}) => {
    ...
    try {
      const opts = { fresh: silent };
      const [s, d] = await Promise.all([fetchStats(opts), fetchLeads({ type, status, owner, q: debounced || undefined, page, page_size: PAGE_SIZE }, opts)]);
      setStats(s);
      setData(d);
    } catch (err) {
