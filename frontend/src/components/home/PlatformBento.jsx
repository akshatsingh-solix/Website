...
export const ProductCard = ({ product, className, large = false }) => {
  ...
  return (
    <Tilt max={5} className={cn("flex", className)}>
      <Link
        to={`/products/${product.slug}`}
        data-testid={`product-card-${product.slug}`}
        className="group relative flex w-full flex-col overflow-hidden rounded-2xl border border-white/10 bg-card p-7 card-hover"
      >
        {large && (
          <>
