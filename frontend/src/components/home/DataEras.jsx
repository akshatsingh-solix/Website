        ...
        <Stagger className="grid grid-cols-2 gap-x-4 gap-y-8 sm:grid-cols-3 lg:grid-cols-6 lg:gap-5" stagger={0.07}>
          {eras.map((e, i) => (
            <Item key={e.era} className="flex">
              <Link to={e.to} className="group flex w-full flex-col rounded-2xl p-1 lg:p-0" data-testid={`era-${ERAS[i].era.toLowerCase()}`}>
                ...
                <span className="mt-3 inline-flex items-center gap-1 text-xs font-medium text-muted-foreground transition-colors group-hover:text-primary-ink">{tx("Learn more")} <ArrowRight className="h-3 w-3 transition-transform group-hover:translate-x-0.5" /></span>
              </Link>
            </Item>
          ))}
        </Stagger>
