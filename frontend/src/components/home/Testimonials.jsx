        ...
        <Reveal className="grid gap-10 lg:grid-cols-12" onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)}>
          <div className="lg:col-span-3">
            ...
            <div className="mt-10 flex items-center gap-2">
              <button onClick={() => setI((i - 1 + TESTIMONIALS.length) % TESTIMONIALS.length)} aria-label={tx("Previous testimonial")} data-testid="testimonial-prev" className="grid h-10 w-10 place-items-center rounded-full border border-line/15 bg-background transition-colors hover:border-primary/40 hover:text-primary-ink">
                <ChevronLeft className="h-4 w-4" />
              </button>
              <button onClick={() => setI((i + 1) % TESTIMONIALS.length)} aria-label={tx("Next testimonial")} data-testid="testimonial-next" className="grid h-10 w-10 place-items-center rounded-full border border-line/15 bg-background transition-colors hover:border-primary/40 hover:text-primary-ink">
                <ChevronRight className="h-4 w-4" />
              </button>
            </div>
            <div className="mt-6 flex gap-1.5">
              {TESTIMONIALS.map((_, idx) => (
                <button key={idx} onClick={() => setI(idx)} aria-label={tx("Go to testimonial {{n}}", { n: idx + 1 })} className={cn("h-1 rounded-full transition-[width,background-color] duration-300", idx === i ? "w-8 bg-primary" : "w-3 bg-line/20")} />
              ))}
            </div>
          </div>
            ...
            <AnimatePresence mode="wait">
              ...
              >
                ...
                <footer className="mt-8 flex items-center gap-4">
                  <span className="grid h-11 w-11 place-items-center rounded-full bg-teal/10 font-mono text-xs font-semibold text-teal">{SOURCE.TESTIMONIALS[i].role.split(" ").filter((w) => /^[A-Z]/.test(w)).map((w) => w[0]).slice(0, 2).join("")}</span>
                  <div>
