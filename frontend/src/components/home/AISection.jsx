...
export const AISection = () => {
  ...
  const checks = useLocalized(CHECKS);
  return (
          ...
          <div className="absolute bottom-4 left-4 right-4 rounded-2xl border border-line/15 bg-background/80 p-4 backdrop-blur-md sm:bottom-5 sm:left-5 sm:right-5">
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1 text-sm">
                <p className="text-foreground">“{tx("Which retired SAP contracts expire in Q3 with auto-renew clauses?")}”</p>
                <p className="mt-1.5 font-mono text-[10px] uppercase tracking-[0.16em] text-teal">{tx("answered from 3 preserved systems · policy-checked")}</p>
              </div>
            </div>
          </div>
        </div>
